...
def get_running_processes_count() -> int:
    """Conta o número de processos em execução (Simplificado)."""
    try:
        if os.name == 'nt':
            output = os.popen('tasklist /NH').read()
...
def get_running_services_count() -> int:
    """Conta o número de serviços ativos (Simplificado)."""
    try:
        if os.name == 'nt':
            output = os.popen('sc query state= all').read()
            return output.count("SERVICE_NAME")
        return 0
    except Exception:
        return 0

def audit_windows_security_simplified() -> dict:
    """Executa uma auditoria de segurança simplificada (TIER 0)."""
    machine = get_machine_info()
    
    # Hardening básico
    hardening = {"firewall_enabled": False}
    try:
        fw = os.popen("netsh advfirewall show allprofiles state").read()
        if "ON" in fw.upper() or "LIGADO" in fw.upper():
            hardening["firewall_enabled"] = True
    except:
        pass
    ...
    return {
        "machine": {
            ...
            "os": machine.get("os"),
            "ip": machine.get("ip_primary")
        },
        "hardening": hardening,
        "counts": {
            "processes": get_running_processes_count(),
            "services": get_running_services_count()
        },
        "timestamp": datetime.now().isoformat()
    }
