...
async def run_scan(module_type):
    """
    Endpoint da API para executar scans de sistema simulados.
    """
    if module_type.lower() == 'vault':
        return jsonify({'error': f'Módulo {module_type} indisponível no TIER 0.'}), 403
...
# Endpoints Restritos no TIER 0
@api_bp.route('/correlation/alerts', methods=['POST'])
async def correlation_alerts():
    return jsonify({'error': 'Módulo de Correlação de Alertas indisponível no TIER 0.'}), 403

@api_bp.route('/analyze/vault', methods=['POST'])
async def analyze_vault():
    return jsonify({'error': 'Módulo Windows Vault indisponível no TIER 0.'}), 403
